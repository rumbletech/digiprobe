...
int32_t EP_Config_S::readRXBuff ( uint8_t* d_ptr   )
{
	//check if ep is allocated and initialized
	if ( !pma.is_alloc( this->EP_Num )  )
	{
		return -1 ;
	}

	uint16_t len = pma.read_hw( this->EP_Num * 8UL + 6UL );
	len&= ( USB_COUNT0_RX_COUNT0_RX_Msk);

	for ( int i = 0 ; i < len ; i++ )
	{
		*(d_ptr++)= pma.read_b( this->EP_RX0_Ptr + i );
	}
