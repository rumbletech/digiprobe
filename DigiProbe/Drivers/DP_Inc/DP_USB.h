...
public :
 ...
 volatile uint16_t * getEPReg ( void );
 ...
 inline bool getRXDTOG( void );
 inline bool getTXDTOG( void );
 inline EP_stat_E getTXStat( void );
 inline EP_stat_E getRXStat( void );
 int32_t writeTXBuff ( uint8_t* d_ptr , uint16_t d_len  );
 int32_t readRXBuff  ( uint8_t* d_ptr  );
...
public:
 ...
 PMA( void );
 void write_hw( uint16_t val , uint16_t ptr );
 uint16_t read_hw( uint16_t ptr );
 uint16_t read_b(  uint16_t  ptr );
 int32_t PMA_allocate( EP_Config_S& ep );
 bool is_alloc( int32_t ep_num  );
